...
def make_parser():
    ...
    parser.add_argument("-o", "--output_dir", default="../../DiffusionDet/output_SmileTrack", type=str, help="input your desired output folder for experiment results")
    ...
    # Detector
    parser.add_argument("--detection_folder", "--detection-folder", default="../../DiffusionDet/output_mot17_detections/FINAL_mot/train", type=str, help="folder with detections files in MOT17Det format")
...
#################
        ...
        fltr = predictor[:,0]==frame_id
        outputs = predictor[fltr,:7]

        outputs[:,[0,1,2,3,4,5]] = outputs[:,[2,3,4,5,6,1]]
        outputs = outputs[:,:6]
        outputs[:,2]=outputs[:,2]+outputs[:,0]
        outputs[:,3]=outputs[:,3]+outputs[:,1]
        outputs[:,5] = -outputs[:,5]
        #input shape 
        #output shape 5 : x1, y1, w, h, score, class
        ...
        img, ratio = preproc(img, exp.test_size, (0.485, 0.456, 0.406), (0.229, 0.224, 0.225))
        img_info["ratio"] = ratio
        
        #print("img shape 2 :",img.shape)
        #print("img_info :", img_info["raw_img"].shape)
...
############
    detections = np.loadtxt(args.detection_file,delimiter=',')
...
########### only FRCNN ?
    ...
    print("Detection files :",det_files)
    ...
    for ext in seqs_ext:
        ...
        for i in seqs:
            ...
            args.detection_file=det_files[j]
            j+=1
            ...
            if args.default_parameters:
                ...
                if seq == 'MOT17-05-FRCNN' or seq == 'MOT17-06-FRCNN':
                    ...
                    #args.exp_file = r'./yolox/exps/example/mot/mot17_exp_mot5.py'
